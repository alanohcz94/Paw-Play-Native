...
import { calculateScore, DIFFICULTY_WINDOW, type CommandResult } from "@/utils/scoring";
...
export default function DemoScreen() {
  ...
  const window = DIFFICULTY_WINDOW[DEMO_DIFFICULTY];
  ...
  const resetTimer = () => {
    ...
    timerRef.current = setInterval(() => {
      ...
      const remaining = window - elapsedRef.current;
      if (remaining <= 0) {
  ...
  const handleHoldEnd = () => {
    ...
    const result: CommandResult = {
      name: currentCommand,
      success: elapsed <= window,
      skipped: false,
      timeSeconds: elapsed,
      windowSeconds: window,
      secondsOver: Math.max(0, elapsed - window),
      pointsEarned: elapsed <= window ? 20 : 20 - Math.floor(elapsed - window),
    };

    const newResults = [...commandResults, result];
    setCommandResults(newResults);
    advanceCommand(newResults, false);
  };
  ...
  const handleSkip = () => {
    ...
    const elapsed = elapsedRef.current;
    const secondsOver = Math.max(0, elapsed - window);
    const result: CommandResult = {
      name: currentCommand,
      success: false,
      skipped: true,
    ...
    const newResults = [...commandResults, result];
    setCommandResults(newResults);
    advanceCommand(newResults, true);
  };

  const advanceCommand = (results: CommandResult[], _skipped: boolean) => {
    const currentScore = results.reduce((s, r) => s + r.pointsEarned, 0);
        ...
        <View style={styles.commandsList}>
          {DEMO_COMMANDS.map((cmd, i) => (
            <View key={cmd} style={[styles.commandRow, { borderColor: colors.border }]}>
              ...
              <Text style={[styles.commandDuration, { color: colors.peach, fontFamily: "Nunito_700Bold" }]}>{window}s</Text>
            </View>
          ))}
        </View>
        <TouchableOpacity
    ...
    return (
      <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0) }]}>
        ...
        <Text style={[styles.timerText, { color: windowExceeded ? "#ef4444" : colors.mutedForeground, fontFamily: "Nunito_700Bold" }]}>
          {windowExceeded ? `${(elapsedRef.current - window).toFixed(1)}s over` : `${timeRemaining.toFixed(1)}s remaining`}
        </Text>
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top + (Platform.OS === "web" ? 67 : 0), paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 0) }]}>
      ...
      <Text style={[styles.endScore, { color: colors.peach, fontFamily: "FredokaOne_400Regular" }]}>{score}</Text>
