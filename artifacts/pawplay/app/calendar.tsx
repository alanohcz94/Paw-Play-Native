...
export default function CalendarScreen() {
  ...
  const loadDaySessions = async (dateStr: string) => {
    ...
    try {
      const { authedFetch } = await import("@/lib/authedFetch");
      const res = await authedFetch(
        `/api/sessions?dogId=${dog.id}&date=${dateStr}`,
      );
      if (res.ok) {
        const data = await res.json();
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                ...
                !data?.trainedByMe &&
                  !data?.trainedByFriends &&
                  !todayCell &&
                  !future && { backgroundColor: `${colors.muted}40` },
              ]}
        ...
        >
          ...
          {loadingDay ? (
            ...
            daySessions.map((s, idx) => {
              ...
              const totalReps = s.commandsUsed.reduce(
                (sum, c) => sum + (c.count ?? 1),
                0,
              );
              return (
                ...
                >
                  <View
                    style={[
                      styles.sessionIconBadge,
                      { backgroundColor: dc.bg },
                    ]}
                  >
                    <Feather
                      name={modeIcon(s.mode) as any}
                      size={16}
                      color={dc.fg}
                    />
                  </View>
                  <View style={styles.sessionBody}>
                    <View style={styles.sessionTitleRow}>
                      ...
                      {s.difficulty && (
                        <View
                          style={[styles.diffChip, { backgroundColor: dc.bg }]}
                        >
                          <Text
                            style={[
                              styles.diffChipText,
                              { color: dc.fg, fontFamily: "Nunito_700Bold" },
                            ]}
                          >
                  ...
                  <Text
                    style={[
                      styles.sessionPts,
                      {
                        color: pts >= 0 ? colors.mint : "#ef4444",
